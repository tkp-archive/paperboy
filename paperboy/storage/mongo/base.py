...
class BaseMongoStorageMixin(object):
    ...
    def _search(self, MongoCls, ClsName, user, params, session, *args, **kwargs):
        '''Search for a given name in instances of ClsName stored in Mongo as type SqlCls

        Args:
            SqlCls (BaseMongoStorageMixin + BaseStorage): Class implementing the BaseStorage interface
        ...
        if name is None or name == '':
            return []
        nbs = session.query(MongoCls) \
            .filter(MongoCls.name.like(lookfor(name))) \
            .filter(MongoCls.userId == int(user.id) or
                    (hasattr(MongoCls, 'privacy') and MongoCls.privacy == 'public')) \
            .limit(count)
